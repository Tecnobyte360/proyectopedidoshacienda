...
function initChatDemo() {
    ...
    window.Echo.channel("demo-channel").listen("MessageSent", (e) => {
        ...
        wrapper.className = isAi ? "flex justify-start mb-2" : "flex justify-end mb-2";
        wrapper.innerHTML = `
...
function initPedidosRealtime() {
    ...
    function callLivewire(method, id) {
        ...
        if (!component) {
            ...
            console.error(
                ...
                all.map((c) => c.name ?? c.snapshot?.memo?.name ?? "(sin nombre)"),
            );
            return;
        }
    ...
    function highlightRow(row) {
        ...
        setTimeout(() => row.classList.remove("new-order-row", "new-order-ring"), 5000);
    }
    ...
    window.Echo.channel("pedidos")
        ...
        .listen(".pedido.actualizado", (event) => {
            ...
            if (event.accion === "cancelado" || event.estado === "cancelado") {
                ...
                showToast(`Pedido #${event.id} actualizado a: ${getEstadoCfg(event.estado).label}.`);
            }
        });
}