    ...
    <div x-data x-show="$wire.modalAbierto" x-cloak wire:ignore.self
        ...
        <div @click.stop class="w-full max-w-3xl rounded-2xl bg-white shadow-2xl my-8 overflow-hidden">
                ...
                <form wire:submit.prevent="guardar" class="p-6 space-y-5 max-h-[75vh] overflow-y-auto">
                    ...
                    {{-- 💳 Bold por tenant (pagos en línea — alternativa a Wompi) --}}
                    <div class="rounded-xl border border-orange-200 bg-orange-50/40 p-4 space-y-3">
                        <div class="flex items-center gap-2 mb-1">
                            ...
                            <div class="flex-1">
                                <h4 class="font-bold text-slate-800 text-sm">Pasarela de pagos · Bold</h4>
                                <p class="text-[11px] text-slate-500">
                                    Alternativa o complemento a Wompi. Llaves cifradas en BD.
                                    <a href="https://developers.bold.co/payments/payment-link" target="_blank"
                                       class="text-orange-700 underline">Cómo obtenerlas en Bold</a>.
                                </p>
                            </div>
                        </div>

                        <label class="flex items-start gap-3 rounded-xl bg-white border border-orange-200 p-3 cursor-pointer">
                            <input type="checkbox" wire:model.live="bold_activo" class="mt-0.5 rounded border-orange-300 text-orange-500">
                            <div>
                                <div class="text-sm font-semibold text-slate-800">Activar Bold para este tenant</div>
                                <div class="text-xs text-slate-500">Si está apagado, no se generan links de Bold y se ignoran sus webhooks.</div>
                            </div>
                        </label>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                            <div>
                                <label class="block text-xs font-semibold text-slate-700 mb-1">Modo</label>
                                <select wire:model="bold_modo"
                                        class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm focus:border-orange-400 focus:ring-2 focus:ring-orange-100 bg-white">
                                    <option value="sandbox">Sandbox (pruebas)</option>
                                    <option value="production">Producción (real)</option>
                                </select>
                            </div>
                            <div class="flex items-end">
                                @if($bold_api_key || $bold_webhook_secret)
                                    <span class="inline-flex items-center gap-1.5 rounded-full bg-emerald-100 text-emerald-700 px-3 py-1 text-[11px] font-bold">
                                        <i class="fa-solid fa-circle-check"></i> Bold configurado
                                    </span>
                                @else
                                    <span class="inline-flex items-center gap-1.5 rounded-full bg-slate-200 text-slate-600 px-3 py-1 text-[11px] font-bold">
                                        <i class="fa-solid fa-circle"></i> Sin configurar
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                            <div>
                                <label class="block text-xs font-semibold text-slate-700 mb-1">
                                    API Key <span class="text-slate-400 font-normal">(x-api-key)</span>
                                </label>
                                <input type="password" wire:model="bold_api_key"
                                       placeholder="Dejar vacío para no cambiar"
                                       autocomplete="off" spellcheck="false"
                                       class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm font-mono focus:border-orange-400 focus:ring-2 focus:ring-orange-100">
                                @error('bold_api_key') <p class="text-xs text-rose-600 mt-1">{{ $message }}</p> @enderror
                            </div>

                            <div>
                                <label class="block text-xs font-semibold text-slate-700 mb-1">
                                    Secret Key <span class="text-slate-400 font-normal">(opcional)</span>
                                </label>
                                <input type="password" wire:model="bold_secret_key"
                                       placeholder="Dejar vacío para no cambiar"
                                       autocomplete="off" spellcheck="false"
                                       class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm font-mono focus:border-orange-400 focus:ring-2 focus:ring-orange-100">
                                @error('bold_secret_key') <p class="text-xs text-rose-600 mt-1">{{ $message }}</p> @enderror
                            </div>

                            <div class="md:col-span-2">
                                <label class="block text-xs font-semibold text-slate-700 mb-1">
                                    Webhook Secret <span class="text-slate-400 font-normal">(para validar firma HMAC-SHA256)</span>
                                </label>
                                <input type="password" wire:model="bold_webhook_secret"
                                       placeholder="Dejar vacío para no cambiar"
                                       autocomplete="off" spellcheck="false"
                                       class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm font-mono focus:border-orange-400 focus:ring-2 focus:ring-orange-100">
                                <p class="text-[10px] text-slate-500 mt-1">Se valida el header <code>x-bold-signature</code> en cada evento entrante.</p>
                                @error('bold_webhook_secret') <p class="text-xs text-rose-600 mt-1">{{ $message }}</p> @enderror
                            </div>
                        </div>
                        ...
                        @if($slug)
                            @php
                                $boldWebhookUrl = url('/api/bold/webhook/' . $slug);
                            @endphp
                            <div class="rounded-xl bg-white border-2 border-orange-300 p-3"
                                 x-data="{ copiado: false }">
                                <div class="flex items-center gap-2 mb-2">
                                    <i class="fa-solid fa-link text-orange-600"></i>
                                    <span class="text-xs font-bold text-slate-800">URL de webhook para Bold</span>
                                </div>
                                <p class="text-[11px] text-slate-600 mb-2">
                                    Pega en tu dashboard de Bold (<strong>Integraciones → Webhooks</strong>).
                                </p>
                                <div class="flex items-center gap-2">
                                    <input type="text" readonly value="{{ $boldWebhookUrl }}"
                                           id="bold-webhook-url-{{ $slug }}"
                                           class="flex-1 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-mono text-slate-700"
                                           onclick="this.select()">
                                    <button type="button"
                                            @click="navigator.clipboard.writeText('{{ $boldWebhookUrl }}'); copiado = true; setTimeout(() => copiado = false, 1500)"
                                            class="inline-flex items-center gap-1.5 rounded-lg bg-orange-600 hover:bg-orange-700 text-white text-xs font-bold px-3 py-2 transition flex-shrink-0">
                                        <template x-if="!copiado"><span><i class="fa-solid fa-copy"></i> Copiar</span></template>
                                        <template x-if="copiado"><span><i class="fa-solid fa-check"></i> ¡Copiado!</span></template>
                                    </button>
                                </div>
                    ...
                    <div class="rounded-xl border border-indigo-200 bg-indigo-50/40 p-4 space-y-2">
                        <div class="flex items-center gap-2 mb-1">
                            ...
                            <h4 class="font-bold text-slate-800 text-sm">Preferencia de pasarela</h4>
                        </div>
                        <p class="text-[11px] text-slate-600">
                            Si configuras Wompi <strong>y</strong> Bold, elige cómo cobrarle al cliente.
                            Si solo activas una, esto se ignora.
                        </p>
                        <select wire:model="pasarela_preferida"
                                class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 bg-white">
                            <option value="cliente_elige">🤝 El cliente elige (mostrar ambos botones)</option>
                            <option value="wompi">💜 Solo Wompi (aunque Bold esté activo)</option>
                            <option value="bold">🟠 Solo Bold (aunque Wompi esté activo)</option>
                        </select>
                    </div>
